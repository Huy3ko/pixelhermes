...
| Companion Stack (Sprint 7, historisch) | [HONCHO.md](HONCHO.md), [COMPANION_STACK.md](COMPANION_STACK.md) | Selbstgehosteter Honcho-Memory-Server + lokaler Embedding-Server (llama.cpp), real installiert, konfiguriert und Ende-zu-Ende verifiziert (Cross-Session-Recall bestätigt) — Honcho seit Phase X vollständig entfernt, siehe Memory-Migration-Zeile unten |
| Google Workspace Skill (recherchiert, nicht aktiviert) | [GOOGLE_WORKSPACE.md](GOOGLE_WORKSPACE.md) | OAuth-/PKCE-Setupablauf, benötigte Google-Cloud-APIs, Scopes je Dienst (Gmail/Calendar/Drive/Sheets/Docs), Token-Ablage — Aktivierung erfordert realen Zugriff auf den `hermes_hugo`-Host und eine Google-Kontoanmeldung des Nutzers, daher nicht Teil dieser Recherche |
...
| Memory-Migration (Phase X) | [MNEMOSYNE.md](MNEMOSYNE.md) | Honcho real durch `mnemosyne-hermes` (Drittanbieter, ADR 0008) ersetzt: offiziell installiert, Duplicate-Registration-Frage per Quellcode und DB-Abfrage widerlegt, 12 Ende-zu-Ende-Tests, realer Performance-Vergleich (Mnemosyne-Recall ⌀29ms vs. Honchos `honcho_reasoning` 23,6s), kein offizieller Migrationsweg (dokumentiert, nicht selbst gebaut), Honcho deaktiviert |
